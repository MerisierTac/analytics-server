...
public class AnalyticsDeletionValidationTest {
    ...
    public void itShouldRejectAnalyticsDeletionWithoutInstallationUuid() throws Exception {
        ...
        checkResult(mvcResult, "Required String parameter 'installationUuid' is not present", OffsetDateTime.now(ZoneId.of("UTC")) );
    }
    ...
    public void itShouldRejectAnalyticsDeletionWithEmptyInstallationUuid() throws Exception {
        ...
        final MvcResult mvcResult = mockMvc.perform(delete("/api/v1/analytics")
                .queryParam("installationUuid", ""))
                .andExpect(status().isBadRequest())
                .andReturn();

        checkResult(mvcResult, "deleteAnalytics.installationUuid: size must be between 1 and 64", OffsetDateTime.now(ZoneId.of("UTC")) );
    }
