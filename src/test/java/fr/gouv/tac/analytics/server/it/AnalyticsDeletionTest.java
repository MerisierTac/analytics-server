...
public class AnalyticsDeletionTest {
    ...
    public void itShouldStoreValidDeletionOrders() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/analytics")
                ...
                .queryParam("installationUuid", installationUuid))
                .andExpect(status().isNoContent())
                .andExpect(content().string(is(emptyString())));


        await().atMost(QUEUE_READ_TIMEOUT, SECONDS)
               .untilAsserted(() -> assertThat(records).isNotEmpty());
        ...
        assertThat(analyticsDeletionResult.getDeletionTimeStamp()).isBetween(Instant.now().minusSeconds(5), Instant.now());
